...
describe("Insure: Close Policy: price falls, taker wins", function () {
  ...
  it("Set oracle price (taker wins)", async function () {
    // Blockchain time to endtime for all policies
    await ethers.provider.send("evm_setNextBlockTimestamp", [
      expirationTimes[2] + 1,
    ]);

    let bellowSecondLevel = ethers.utils.parseEther("199");
    await oracleContract.setExpiryPrice(secondExpiry, bellowSecondLevel);
  ...
  it("Right balance provider", async function () {
    let expectedBalance =
      1000 + normalSecondPrice * secondLots - secondLots * 100;
  ...
  it("Right balance taker", async function () {
    ...
    expect(actualBalance.toFixed(2)).to.equal(expectedBalance.toFixed(2));
  });
